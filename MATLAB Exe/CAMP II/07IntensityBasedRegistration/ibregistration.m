...
similarity_measure = 'NCC';         % SSD, SAD, NCC or MI
iterSimplex = 0;
...
[transform_params, min_value,exitflag] = fminsearch(@(x) cost_function(x, img_fixed, img_moving, similarity_measure),initial_params,options);
%% output function
...
temp = image_rotate( img_moving, angle , [0 0]);

img_registered = image_translate(temp,[dx dy]);
